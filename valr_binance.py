...
def make_profit(binance_data, valr_data):
    ...
    for base_asset in base_assets:
        for first_change in valr_pairs:
            if base_asset in first_change and 'ZAR' in first_change:
                if first_change.endswith('ZAR'):
                    first_value = 1.0 / valr_data[first_change]
                elif first_change.startswith('ZAR'):
                    first_value = valr_data[first_change]
                for second_change in valr_pairs:
                    if second_change.endswith(base_asset):
                        temp_asset = second_change[:-len(base_asset)]
                        if f'{temp_asset}ZAR' not in binance_pairs:
                            continue
                        else:
                            if(valr_data[second_change] == 0):
                                continue
                            second_value = first_value / valr_data[second_change]
                            last_value = second_value * binance_data[f'{temp_asset}ZAR']
                            if last_value > 1:
                                print("ZAR->" + base_asset + "("+"{:f}".format(valr_data[f'{base_asset}ZAR']) + ")"+"->" + temp_asset + "("+"{:f}".format(valr_data[f'{temp_asset}{base_asset}']) +")"+ "->ZAR" + "("+"{:f}".format(binance_data[f'{temp_asset}ZAR']) +")"+ ": profit=" + "{:f}".format(last_value-1))
                        ...
                        if f'{temp_asset}ZAR' not in binance_pairs:
                            ...
                            last_value = second_value * binance_data[f'{temp_asset}ZAR']
                            if last_value > 1:
                                print("ZAR->" + base_asset + "("+"{:f}".format(valr_data[f'{base_asset}ZAR']) + ")"+"->" + temp_asset + "("+"{:f}".format(valr_data[f'{base_asset}{temp_asset}'])+")" + "->ZAR" + "("+"{:f}".format(binance_data[f'{temp_asset}ZAR']) +")"+ ": profit=" + "{:f}".format(last_value-1))
                for second_change in valr_pairs:
                    if second_change.endswith(base_asset):
                        X_asset = second_change[:-len(base_asset)]
                        for third_change in valr_pairs:
                            if X_asset in third_change:
                                if third_change.startswith(X_asset):
                                    Y_asset = third_change[len(X_asset):]
                                    if f'{Y_asset}ZAR' not in binance_pairs:
                                        continue
                                    else:
                                        if(valr_data[second_change] == 0):
                                            continue
                                        second_value = first_value / valr_data[second_change]
                                        third_value = second_value * valr_data[third_change]
                                        last_value = third_value * binance_data[f'{Y_asset}ZAR']
                                        if last_value > 1:
                                            print("ZAR->" + base_asset + "("+"{:f}".format(valr_data[f'{base_asset}ZAR']) + ")"+"->" + X_asset + "("+"{:f}".format(valr_data[f'{X_asset}{base_asset}']) +")"+ "->"+ Y_asset+ "("+"{:f}".format(valr_data[f'{X_asset}{Y_asset}'])+")"+"->ZAR" + "("+"{:f}".format(binance_data[f'{Y_asset}ZAR']) +")"+ ": profit=" + "{:f}".format(last_value-1))
                                elif third_change.endswith(X_asset):
                                    Y_asset = third_change[:-len(X_asset)]
                                    if f'{Y_asset}ZAR' not in binance_pairs:
                                        continue
                                    else:
                                        if(valr_data[second_change] == 0 or valr_data[third_change] == 0):
                                            continue
                                        second_value = first_value / valr_data[second_change]
                                        third_value = second_value / valr_data[third_change]
                                        last_value = third_value * binance_data[f'{Y_asset}ZAR']
                                        if last_value > 1:
                                            print("ZAR->" + base_asset + "("+"{:f}".format(valr_data[f'{base_asset}ZAR']) + ")"+"->" + X_asset + "("+"{:f}".format(valr_data[f'{X_asset}{base_asset}']) +")"+ "->"+ Y_asset+ "("+"{:f}".format(valr_data[f'{Y_asset}{X_asset}'])+")"+"->ZAR" + "("+"{:f}".format(binance_data[f'{Y_asset}ZAR']) +")"+ ": profit=" + "{:f}".format(last_value-1))
                            ...
                            if X_asset in third_change:
                                if third_change.startswith(X_asset):
                                    ...
                                    if f'{Y_asset}ZAR' not in binance_pairs:
                                        ...
                                        last_value = third_value * binance_data[f'{Y_asset}ZAR']
                                        if last_value > 1:
                                            print("ZAR->" + base_asset + "("+"{:f}".format(valr_data[f'{base_asset}ZAR']) + ")"+"->" + X_asset + "("+"{:f}".format(valr_data[f'{base_asset}{X_asset}']) +")"+ "->"+ Y_asset+ "("+"{:f}".format(valr_data[f'{X_asset}{Y_asset}'])+")"+"->ZAR" + "("+"{:f}".format(binance_data[f'{Y_asset}ZAR']) +")"+ ": profit=" + "{:f}".format(last_value-1))
                                elif third_change.endswith(X_asset):
                                    Y_asset = third_change[:-len(X_asset)]
                                    if f'{Y_asset}ZAR' not in binance_pairs:
                                        continue
                                    else:
                                        if(valr_data[third_change] == 0):
                                            continue
                                        second_value = first_value * valr_data[second_change]
                                        third_value = second_value / valr_data[third_change]
                                        last_value = third_value * binance_data[f'{Y_asset}ZAR']
                                        if last_value > 1:
                                            print("ZAR->" + base_asset + "("+"{:f}".format(valr_data[f'{base_asset}ZAR']) + ")"+"->" + X_asset + "("+"{:f}".format(valr_data[f'{base_asset}{X_asset}']) +")"+ "->"+ Y_asset+ "("+"{:f}".format(valr_data[f'{Y_asset}{X_asset}'])+")"+"->ZAR" + "("+"{:f}".format(binance_data[f'{Y_asset}ZAR']) +")"+ ": profit=" + "{:f}".format(last_value-1))
...
def _main():
    ...
    # valr_pairs = set(binance_data.keys())
    # for i in valr_data:
    #     if 'BTCZAR' in i:
